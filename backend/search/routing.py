...
# Signals that rest on a bare temporal word alone. These are ambiguous: "last
# month" attaches equally to an information need ("what shipped last month") and
# to a personal statement ("I graduated last month"). The difference is intent,
# not vocabulary, so no pattern can separate them - the classifier must. A weak
# signal is therefore authoritative only when nothing marks the query as being
# about the user (see below); otherwise it defers.
_WEAK_TEMPORAL = frozenset({"recency_term", "time_term", "relative_period"})

# Reason returned when a weak signal is present but the query is about the user,
# so the deterministic layer abstains and the cascade consults the classifier.
DEFER_REASON = "ambiguous_self_reference"

# Self-reference is the one thing here that is finite and stable, unlike the
# unbounded ways a person can phrase a statement about their life. Its presence
# alongside only a weak temporal signal is what marks the ambiguous zone; the
# actual intent judgement is left to the classifier, not enumerated in regex.
...
class SearchRoutingPolicy:
    ...
    def decide(self, query: str) -> SearchDecision:
        ...
        # A strong signal names a volatile topic and wins immediately, even in a
        # sentence about the user. A weak temporal signal is only remembered,
        # because whether it warrants a search depends on intent.
        ...
        if weak_match or year_hit:
            # Ambiguous: a bare time word about the user is not a web query, but
            # only the classifier can tell "I graduated last month" from "what
            # shipped last month". Abstain so the cascade decides; without a
            # self-reference the temporal signal stands on its own.
            if _SELF_REFERENCE.search(query):
                return SearchDecision(should_search=False, reason=DEFER_REASON)
            reason = weak_match or "current_or_future_year"
            return SearchDecision(should_search=True, reason=reason)
