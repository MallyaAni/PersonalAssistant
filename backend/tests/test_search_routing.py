...
# The assistant knew what it could do and nothing about what it was. Asked what
# to host on one DGX Spark for chat and image work, it recommended models from
# memory and never mentioned the ones already answering the question.
def test_the_prompt_names_the_models_actually_serving_the_conversation():
    prompt = _build_system_prompt(
        {
            "runtime": [
                {"role": "Writing this reply", "model": "deepseek-v4-flash"},
                {"role": "Creating pictures", "model": "flux-2-klein-4b"},
            ]
        },
        now=datetime(2026, 8, 18, tzinfo=UTC),
    )

    assert "- Writing this reply: deepseek-v4-flash" in prompt
    assert "- Creating pictures: flux-2-klein-4b" in prompt
    # Without this the model treats the list as trivia and answers from memory.
    assert "outrank your own recollection" in prompt


# Endpoints and hostnames are not the assistant's to volunteer: the prompt
# reaches every account, including invited guests.
def test_no_runtime_block_is_claimed_when_none_is_supplied():
    prompt = _build_system_prompt({}, now=datetime(2026, 8, 18, tzinfo=UTC))

    assert "The models running AniOS" not in prompt


def test_an_entry_missing_its_model_is_skipped():
    prompt = _build_system_prompt(
        {
            "runtime": [
                {"role": "Reading images", "model": ""},
                {"role": "", "model": "orphan-model"},
                {"role": "Writing this reply", "model": "deepseek-v4-flash"},
            ]
        },
        now=datetime(2026, 8, 18, tzinfo=UTC),
    ...
    assert "Reading images" not in prompt
    assert "orphan-model" not in prompt
