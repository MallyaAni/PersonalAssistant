...
from backend.agents.experience.sources import Turn, render_turns
...
async def _judge(structured_llm, turns):
    return await ExperiencePrompts(structured_llm).judge(render_turns(turns))
...
async def test_the_bird_and_the_reminder_are_both_found_with_words_from_the_exchanges(structured_llm):
    ...
    seen = []
    for _ in range(3):
        judgement = await _judge(structured_llm, _DAY)
