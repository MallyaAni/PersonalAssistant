...
from backend.agents.trading.desk import funded_execution, simulate
...
# A panel whose final column is the SPY benchmark, so the decision context's
# SPY column always agrees with the panel's tradable SPY prices.
def _panel(close, tickers=None) -> Panel:
...
def _report(close=None, grades=None, tickers=None):
...
def test_execution_eligibility_blocks_new_spy_buys_only():
    ...
    blocked = funded_execution.plan_funded(
        decision, held={}, prices={"SPY": 100.0}, equity=100.0, cash=100.0,
        whole_shares=False, index_eligible=False,
    )
    ...
    allowed = funded_execution.plan_funded(
        decision, held={}, prices={"SPY": 100.0}, equity=100.0, cash=100.0,
        whole_shares=False, index_eligible=True,
    )
