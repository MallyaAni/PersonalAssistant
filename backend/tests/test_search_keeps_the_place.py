...
def test_a_whats_on_query_gets_the_days_it_means():
    assert _hold_to_dates("fun events Arlington Virginia", "what's on this week?", WED) == (
        "fun events Arlington Virginia September 2-6 2026"
    )
    assert _hold_to_dates("things to do Arlington", "anything happening tonight?", WED) == (
        "things to do Arlington September 2 2026"
    )
    # A question with a window but no place-bound words is left alone: the
    # dates rule follows the same test the place rule does, so "what did the
    # Fed decide this week?" is not given a date range it never asked for.
    assert _hold_to_dates("Fed decision", "what did the Fed decide this week?", WED) == "Fed decision"
...
def test_a_query_that_already_names_days_and_a_question_with_no_window_are_left_alone():
    ...
    assert _hold_to_dates(already, "what's on this weekend?", WED) == already
    assert _hold_to_dates("PS5 price", "what does a PS5 cost?", WED) == "PS5 price"
    assert _hold_to_dates("jazz nights Arlington", "what jazz nights are there?", WED) == "jazz nights Arlington"
