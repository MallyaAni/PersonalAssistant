...
pinned by: functional/test_reply_graph_behaviour.py, functional/test_evidence_honesty_behaviour.py, functional/test_no_invented_search_behaviour.py, functional/test_task_referent_behaviour.py, functional/test_reply_brevity_behaviour.py, functional/test_reply_no_meta_commentary_behaviour.py
...
The blocks below, and the failure each one exists to prevent:
  ...
  When an earlier message said "tonight"
    Every stored turn now carries the time it was said, in square brackets
    before the speaker (backend/services/transcript.py). It had none until
    2026-08-29, and that afternoon a group was told an ice-cream run set the
    previous evening was happening "tonight" - the words were in the history
    and nothing said how old they were. The stamp is the mechanism; this is
    the sentence that tells the model to read it.
  ...
  Do not comment on the conversation's own mechanics
    A repeated request was answered with commentary on the repetition - a
    remark that the same thing had been asked, or "again?" - instead of the
    redo the person wanted (a group's "try again", 2026-09-07). Commentary on
    the conversation itself is never the answer. Kept general on purpose: the
    incident belongs in this note, not in the text sent to the model.
...
Do not comment on the conversation's own mechanics. Never remark on repetition, your own past answers, or how many times something was asked - "again?", "as I said", "we've been over this" are commentary on the conversation itself, which is never the answer. Whatever the person says, including a repeated request, is a fresh thing to act on or answer: treat it as such and say only what the request needs. Nothing in your own history is worth pointing at.
