...
- The application-owned local capability FastMCP server is configured `untrusted` and
  ...
  database, artifact-volume, LM Studio, ComfyUI, and presentation-renderer
...
- Presentation briefs and selected-slide feedback are sent only to the configured local Gemma endpoint. Each feedback revision also stores the non-secret stable target slide ID needed to reconstruct its owned per-slide conversation. The model sees no storage keys and cannot authorize, persist, render, or promote revisions. A dedicated local renderer receives only a strict validated deck specification; it writes to an isolated temporary directory, serializes LibreOffice jobs, returns bounded PPTX bytes, and cleans the temporary files. PostgreSQL stores user-scoped append-only revision metadata and the canonical specification; the opaque artifact volume stores each revision's PPTX. Owned deletion removes the deck, revision rows, and linked binaries. Automated retention, encrypted/tested backups, package malware scanning, and process isolation remain planned.
...
- `VERIFIED` (bounded, opt-in): AES-256-GCM application-level encryption of conversation, memory, image, and presentation content when `ENCRYPTION_KEY` is set, with lazy plaintext migration and authenticated ciphertext; full-store coverage (embeddings and dedup columns are intentionally excluded), encrypted/tested backups, and a database-native at-rest option remain `PLANNED`;
...
- `VERIFIED` (bounded): presentation inputs use strict typed schemas and size limits; stale-base edits fail before model use; only a validated ready revision can become current; downloads are private/no-store and ownership-scoped; structural OOXML and LibreOffice checks reject malformed output; and model/MCP responses expose no PPTX bytes or storage keys. Package malware scanning, renderer sandboxing, retention, service-to-service authentication, and encrypted/tested backups remain `PLANNED`;
