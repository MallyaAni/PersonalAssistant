...
- Every tool the turn loop can start carries an effect contract (`backend/core/effects.py`): what it does to the world, whether a dropped call may be replayed, whether a person must approve it. A later step of a turn is offered only reads and writes to this system's own records that are not expensive and need no approval; nothing that sends, spends or changes something outside. MCP retries follow the tool's contract, never the server's trust; arguments are validated against the whole declared schema and screened for secrets however deeply nested; an operator may narrow a tool's policy per server (`tools` in `MCP_SERVERS_JSON`) but never widen it past the classification.
- Durable runs (`docs/RUNS_ARCHITECTURE.md`) act under a principal and record every step before it runs; an approval is bound to the hash of one exact call and spent once; a person's cancel is honoured between steps; the runs routes need `runs:read` to look and `runs:act` to cancel or approve, and isolation is pinned in `test_runs_api_isolation.py`. The security agent refuses any asset not in `SECURITY_AUTHORIZED_ASSETS` before a tool is called, and its only tools are reads of the repository the `repo` MCP server is rooted at by environment.
- A token may be restricted to least-privilege scopes (`chat`, `memory:read`, `memory:write`, `tools:invoke`, `vision`, `runs:read`, `runs:act`, or the `memory`/`tools`/`runs` groups). A route requires the scope matching its action - a read needs `memory:read`, a write needs `memory:write` - and a token lacking it returns 403 before the handler runs. A token with no scope claim (the default, and every token issued before scopes existed) stays unrestricted, so scopes narrow a token without a migration. Scopes limit what a valid token can reach; they are not a substitute for the ownership check, which still binds every request to the token subject.
...
- Chat system instructions, the current prompt, and up to the configured number of same-user prior conversation turns are sent over the house LAN to the DeepSeek server on the Sparks (`animallya-spark1.local:8000`), not to a container on the private Compose network. The chat-completions adapter does not request a provider storage control, so vLLM configuration and process-level logging must be reviewed before sensitive use.
...
- The iMessage bridge is the one inbound message-content ingress. Behind a
  ...
  every other turn). Cursor, dedup, and reaction receipts hold opaque
  identifiers; burst fragments and the last reply remain in Redis for up to a
  day, and up to 100 recent Scout text bubbles retain their body, destination,
  account, optional room context, and Apple GUID for seven days so a later
  ❤️/👍 can be resolved against the exact bubble. Only known GUIDs go back to
  the Mac's read-only reaction tool; no incoming body or conversation listing
  is exposed by it. For a room, the tool returns a reactor only when its address
  is allowlisted; the backend then requires that address to map to a current
  room member and runs the turn as that reactor, preventing one member's
  tapback from borrowing another member's identity. The local routing model
  judges acceptance, failure is closed, and a claimed reaction cannot execute
  twice. Denying a request clears
...
- The routing model may choose at most one action per step from a single native tool-calling call offered by `MainActionSelector`, and a turn takes at most `TURN_MAX_STEPS` steps (3 in this deployment) inside `TURN_STEP_BUDGET_SECONDS`. What each step may be offered is narrowed by the tools' own effect contracts. The call offers: the live `search_web` schema, built-in image/diagram/delegation actions, and a user-scoped semantic shortlist of the user's own tools whose schemas were re-read from locally configured `trusted` or `read_only` MCP servers. A name the call did not actually offer this round is refused before any downstream effect, regardless of what the model returns. The model cannot invoke a toolbox tool directly: AniOS revalidates the live fingerprint, description, schema, arguments, and privacy policy. Consequential servers are not offered to autonomous chat selection.
...
- Validated image bytes and a bounded user prompt are sent over the house LAN to Qwen3-VL on spark2 (`:8001`) for vision analysis. They are not sent to ComfyUI or an internet provider, but vLLM process logging, host access, and retention still require review before sensitive images are used.
...
- `VERIFIED` (local boundary): a loopback-only same-origin Nginx gateway serves
  the compiled UI and proxies API/SSE/upload/download traffic. `VERIFIED`: remote-UI ingress reaches that gateway
  through one TLS hostname, deep-matter.com, over a Cloudflare tunnel run as a
  Compose service on spark1. Cloudflare terminates that TLS, so it carries the
  traffic in the clear and is inside the trust boundary. The edge protects both
  static UI assets and `/api`, validates its session at the origin, proxies SSE
  and bounded uploads/downloads, and forwards only to the same-origin local
  gateway.
...
- `VERIFIED`: semantic Scout-interest capture sends only the current utterance
  to the configured local DeepSeek endpoint, requires grammar-constrained bounded
