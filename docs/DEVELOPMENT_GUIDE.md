...
| Reply length | how long the reply model's answers run on seven fixed questions, before and after a prompt change (the operator's 2026-08-28 complaint: too long) | `docker compose --profile test run --rm --no-deps -v $PWD/backend:/app/backend:ro -v $PWD/prompts:/app/prompts:ro functional-tests python -m backend.cli.measure_reply_length`; held by `functional/test_reply_brevity_behaviour.py` |
...
| Search spend | who is spending the search allowance - people or the sweep's own live questions - before blaming a provider | Redis `anios:search:<user>:<month>` and `:day:<date>` keys; provider totals in `data/search/*.sqlite3`; `search_credits` for the meter |
...
| `PRESENTATION_AUTO_IMAGE_MAX` | `1` | Maximum highest-priority model-declared slide visuals the durable worker generates by itself; add more images per slide on demand, or set `0` to disable |
| `PRESENTATION_AUTO_IMAGE_SIZE` | `1024` | Square pixel size for automatic slide visuals; the default keeps a hero image sharp without the fourfold pixel cost of 2048 |
...
Model servers are started in two different ways, which is worth knowing
before debugging one of them. Compose owns exactly one: `vllm-embedding`,
pinned by image digest with the Nomic revision pinned too. The reply model and
the vision model are systemd units on the Sparks, not containers - see
`deploy/spark/` - so `docker compose ps` will never show them and restarting
Compose will never restart them.

```bash
docker compose up -d --wait --wait-timeout 900 vllm-embedding
curl -s http://127.0.0.1:8004/v1/models              # Nomic, in Compose
curl -s http://animallya-spark1.local:8000/v1/models  # DeepSeek, on the Sparks
curl -s http://animallya-spark2.local:8001/v1/models  # the VLM, on spark2
```

The one-command startup encodes the safe order: wait for the embedding
service, start host ComfyUI, apply migrations, then start the application
services:
...
To move the stack to another host, stop all writers and take a fresh
...
without its key. On the new host, clone the same source revision, restore the
...
The embedding service serves Nomic at 2,048 tokens, sixteen sequences, and
0.06 GPU-memory utilization, sized to its measured 0.26 GiB of weights.

Serving numbers are measured on the profile that produced them, not portable
defaults. Requalify after changing hardware, model, precision, context,
concurrency, or vLLM version. Health and model-list responses prove readiness
only; repeat direct AniOS API, worker, browser, and ComfyUI coexistence
acceptance after such changes.

### The Sparks

Two NVIDIA DGX Sparks (GB10 Grace Blackwell Superchip, 128 GB unified
CPU/GPU memory each, Ubuntu 24.04, sm_121) are the system's only inference
hosts. The first joined on 2026-08-13; the migration off the Windows desktop
finished on 2026-08-23 (`DGX_MIGRATION.md`). DeepSeek-V4-Flash is served
across both of them by `ds4-tp2.sh` under systemd: tensor-parallel 2, a
1,048,576-token context, six concurrent sequences, and 0.81 GPU-memory
utilization, on vLLM 0.25.2. Qwen3-VL-8B serves vision on spark2 at `:8001`.
The desktop keeps only ComfyUI, Docling and Gotenberg, at `172.16.8.6`.

- Hostnames: `animallya-spark1.local` (`172.16.8.3`) and
  `animallya-spark2.local` (`172.16.8.5`), both mDNS. SSH as `animallya96`,
  key-based. Neither has a BMC or wake-on-LAN, so a powered-off Spark needs
  someone to press its button.
- The device's own DGX Dashboard (`dashboard-service -port 11000 serve`)
  binds `127.0.0.1` only, so it is not reachable over the LAN. Reach it with
  an ordinary SSH forward when you want it:
  `ssh -L 11000:127.0.0.1:11000 spark1`. Nothing in the system depends on
  it; it is a convenience for a person looking at the machine.
...
python -m backend.cli.qualify_models \
  --base-url http://animallya-spark1.local:8000 \
...
**Historical, and kept only as a worked example of the harness's output.** None of the models or runtimes below serves a role today. The 2026-07-31 LM Studio RTX 5080 baseline ran three times with Qwen3.5 9B as main,
...
`scripts/start-anios.sh` brings the whole stack up with one command: it waits for `vllm-embedding`, starts host ComfyUI if needed, applies migrations, starts the remaining Compose services, and waits for the backend and for ComfyUI when it launched it. It runs under any Bash, including Git Bash on Windows. Image generation needs ComfyUI running; when it is down, `POST /api/v1/images/generate` returns a `503` with `reason: image_provider_unreachable` and a message naming ComfyUI, which the composer surfaces verbatim.

The verified Windows host uses ComfyUI 0.28.0, Python 3.14, PyTorch CUDA
13.0, the FLUX.2 Klein 9B checkpoint (`flux-2-klein-9b-Q6_K.gguf`) shared by generation and
editing with the Qwen 3 8B text encoder, and the official ComfyUI FLUX.2 Klein
split workflow for editing. Keep
...
hostname no longer changes. In the deployment the connector is not run by hand at
all: `cloudflared` is a Compose service on spark1 and comes back with the rest
of the stack. `docker compose ps cloudflared` and its logs are where to look
when deep-matter.com stops answering.
...
For raster generation, keep the embedding service and ComfyUI running and submit an allowlisted training resolution. The body is piped to `curl.exe` because Windows PowerShell 5 can mishandle large or completed `Invoke-WebRequest` responses:
...
For real image understanding, upload a validated image to the VLM on spark2 (`:8001`):
...
For browser acceptance, also submit `create an image of ...` while Chat is selected. Image generation now runs inside the chat stream — the main model decides it via `MainActionSelector`, not a client-side regex — so require exactly one `/chat` request carrying `artifact_started`/`artifact_ready` events (`kind=generated_image`) and no request to `/images/generate` at all. Then ask a historical question such as `what car did we create an image of?`; require a second `/chat` request with no artifact events, a grounded answer, terminal `done`, and cleared loading/input. Then explicitly ask to search the internet for that image, require image recall before the visible internet MCP lifecycle, and require source cards only when the provider returned non-empty results. Inspect Network and Console throughout. The Memory screen must not fetch the full export before a map-card click; selecting Semantic cache must return the owned export and display a bounded detail region without embedding vectors.
...
Inspect the backend environment for `MEMORY_PROPOSAL_LLM_MODEL=deepseek-v4-flash`
...
as stage-weighted completed work, not an elapsed-time estimate. Deck planning and image
generation still run one after the other, but no longer because they compete
for one card: planning is on the Sparks and ComfyUI is on the desktop. What
keeps them serial now is the image path's own concurrency of one. Overlapping
them needs a tested capacity-aware lease, not just a second GPU.

Generate an image for a selected slide, then submit explicit image feedback in
the same Slide imagery panel. The first request must generate with FLUX.2 Klein and
attach one owned image UUID. The second request must read that exact attached
artifact, edit it with source-conditioned lineage, attach a different child UUID
...
The first opt-in live check requires default slide imagery from the
background worker, refines one selected image, verifies in-place UI
...
| `gate.sh` | ~20 min | four functional files against the real models; `test_tool_selection_matrix_behaviour` runs the whole 124-case routing matrix and dominates |
| build and restart | the remainder | image build, migrations, `up -d`, verify |
| `sweep_journeys` | ~15 min | 55 journeys, each a live turn, run one after another, plus a retry pass for any gap |
