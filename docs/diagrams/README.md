...
| Trading desk | The five tone fields of each earnings release, and the words of a brief. Every rank, grade, weight and order is a rule, measured walk-forward before it was kept; the paper book is reconciled by order id, and the rebalance is the only exit. | [source](agent-trading-desk.mmd) | [view](agent-trading-desk.svg) |
