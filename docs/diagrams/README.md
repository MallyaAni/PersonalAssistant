...
| Document knowledge | How does a shared PDF or Word file become cited answers and attributed memory, what happens while the desktop parser is off, and how does "put that in a PDF" come back as a file? | [document-knowledge.mmd](document-knowledge.mmd) | [document-knowledge.svg](document-knowledge.svg) |
| Market data | How is daily market history fetched without being refused, why is a fetch never rewritten, and what does a learned model have to beat before it is trusted? | [market-data.mmd](market-data.mmd) | [market-data.svg](market-data.svg) |
| Session-price collection | How does backend-owned quote collection publish one dated latest snapshot for read-only dashboard access without changing execution? | [session-price-collection.mmd](session-price-collection.mmd) | [session-price-collection.svg](session-price-collection.svg) |
...
| Trading desk | Adopted scheduled allocation and FOMC paper execution, dated execution eligibility, separate collected display quotes, manual personal positions, and immutable forward-research evaluation. | [source](agent-trading-desk.mmd) | [view](agent-trading-desk.svg) |
...
| Experience review | How does the assistant find where it let someone down - its own recent conversations read by a judge that names each fault, one at a time, with the turn it happened in? | [agent-experience.mmd](agent-experience.mmd) | [agent-experience.svg](agent-experience.svg) |
