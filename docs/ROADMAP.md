...
- `VERIFIED`: free local ComfyUI plus FLUX.2 Klein generates and source-edits images through typed providers and one-job concurrency. Direct RTX 5080 generation completed in about 35 seconds in the RTX 5080 profile of the time, and live browser cancellation interrupts the exact ComfyUI prompt, records `failed/cancelled`, clears loading, and produces no backend exception. Broader quality, crash recovery, and sustained-load benchmarks remain `PLANNED`; paid APIs, subscriptions, credits, and automatic cloud fallback remain excluded.
...
semantic-gated image recall in chat. Cross-modal scores are not comparable to
...
- `VERIFIED` (primary path): a new upload now makes one schema-constrained VLM
  ...
  identities may enter derived visual memory; supported medium and low readings
  are shown as unconfirmed while contradicted readings may be omitted. Safety-sensitive cases
...
- `IN PROGRESS` (accepted design in ADR 0007): initial VLM descriptions and semantic/pixel vectors support owner-scoped image recall. One bounded semantic modality gate now runs before either private index, with aligned pixel lookup first and a description-vector/selection fallback; the prior trigger-word route is retired. Ready FLUX refinements are observed by the local Qwen vision boundary and receive their own derived description/index; observation failure preserves the valid edited artifact and degrades to lineage. Durable idempotent observation for every generated-image path, typed append-only semantics, calibrated alias/conversation/recency fusion, and semantic post-edit verification remain planned.
- `SCAFFOLDED`: unselected image-edit requests now use owner-scoped semantic candidates and a bounded Qwen strict-JSON referent resolver. One match is named and edited; several matches render the owned choices and ask; no match asks for a more specific reference. The resolver/source contract is type-neutral, but only the image source is connected today. Video observations and parsed PDF/RAG chunks remain planned; direct API and browser acceptance for the new unselected-edit path remain unverified.
