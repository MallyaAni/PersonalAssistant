...
export const ForwardEvidence = ({evidence}: {evidence?: DeskForwardEvidence}) => (
  <details className="rounded-2xl border border-black/[0.08] bg-white p-4 text-xs text-[#6e6e73]">
    ...
    <p className="mt-2">First daily signal only. Means use non-overlapping date cohorts; approximate 95% intervals require 20 cohorts. No interval means insufficient evidence for that estimate. The funded target trackers are separate from the complete scheduled strategy and actual paper fills. Costs are assumptions; modeled fills use later observed prices. Recorded splits adjust shares; dividends accrue as receivables and cannot fund buys because pay dates are unavailable. Other corporate actions are not modeled.</p>
