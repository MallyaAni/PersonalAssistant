...
test('renders the desk at a glance with the track record', async ({ page }) => {
  ...
  await expect(glance.getByText('+312.5')).toBeVisible()
  await expect(glance.getByText('The rules, since inception')).toBeVisible()
...
test('drills into a name’s own history', async ({ page }) => {
  ...
  await expect(dialog.getByText('The desk’s rule')).toBeVisible()
  await expect(dialog.getByText('18%', { exact: true })).toBeVisible()
  await expect(dialog.getByText('Just holding it')).toBeVisible()
  await expect(dialog.getByText('The benchmark')).toBeVisible()
