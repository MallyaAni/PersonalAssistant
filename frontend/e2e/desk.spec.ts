...
test('single board keeps cash and wait actions during FOMC', async ({page}) => {
  ...
  await expect(board.locator('tbody tr').filter({has: page.getByRole('button', {name: /^AAPL/})})).toContainText('FOMC cycle: regular trading paused')
  await expect(board.locator('tbody tr').filter({has: page.getByRole('button', {name: /^NVDA/})})).toContainText('FOMC cycle: regular trading paused')
  await expect(board.locator('tbody tr')).toHaveCount(4)
  await page.goto('/?deskDetails=1#desk')
...
test('the desk fits a phone without sideways scrolling', async ({page}) => {
  ...
  // A stock's reason is present beside its quote without horizontal panning.
  ...
  await expect(page.getByLabel('AAPL mobile reason', {exact: true})).toBeVisible()
  await page.getByRole('button', {name: 'details for AAPL', exact: true}).click()
  await expect(page.getByText('Open the full panel')).toBeVisible()
