...
test('extended midpoint is separate from regular signal price on board and chart', async ({page}) => {
  ...
  await expect(board.getByLabel('MSFT session price')).toContainText('No fresh quote from available feeds')
...
test('failed optional feed does not preserve its previous session price', async ({page}) => {
  ...
  await expect(board.getByLabel('AAPL session price')).toContainText('No fresh quote from available feeds')
