...
test('extended midpoint is separate from regular signal price on board and chart', async ({page}) => {
  ...
  await expect(board.getByLabel('MSFT session price')).toContainText('No recent quote to display')
  ...
  await expect(chart.getByLabel('AAPL session price')).toContainText('Price signals use regular-session candles.')
  await expect(chart.locator('dl')).toContainText('$100.00')
  await expect(chart.locator('dl')).not.toContainText('$102.00')
  await page.clock.fastForward(21_000)
  await expect(chart.getByLabel('AAPL session price')).toContainText('No recent quote to display · Last quote:')
  await expect(chart.getByLabel('AAPL session price')).not.toContainText('$102.00')
  await expect(board.getByLabel('AAPL session price')).toContainText('No recent quote to display · Last quote:')
...
test('failed optional feed does not preserve its previous session price', async ({page}) => {
  ...
  await expect(board.getByLabel('AAPL session price')).toContainText('No recent quote to display')
